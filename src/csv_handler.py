...
class CSVHandler:
        ...
        try:
            with open(caminho, 'r', encoding=self.encoding, newline='') as arquivo:
                ...
                if 'username' not in reader.fieldnames:
                    raise ValueError(f"Coluna 'username' não encontrada. "
                                   f"Colunas disponíveis: {list(reader.fieldnames)}")
                
                if 'password' not in reader.fieldnames:
                    raise ValueError(f"Coluna 'password' não encontrada. "
                                   f"Colunas disponíveis: {list(reader.fieldnames)}")
                
                # Lê as credenciais linha por linha
                for linha_num, linha in enumerate(reader, start=2):  # start=2 porque linha 1 é o cabeçalho
                    ...
                    if usuario and senha and usuario != '' and senha != '':
                        ...
                        if len(usuario) < 3:
                            linhas_ignoradas += 1
                            logger.warning(f"⚠️  Linha {linha_num} ignorada: username muito curto (mínimo 3 caracteres)")
                            continue
                        
                        if len(senha) < 1:
                            linhas_ignoradas += 1
                            logger.warning(f"⚠️  Linha {linha_num} ignorada: password vazio")
                            continue
                        
                        # Ignora passwords que são placeholders
                        if senha in ['[NOT_SAVED]', '[REDACTED]', '[HIDDEN]', 'N/A', 'NULL']:
                            linhas_ignoradas += 1
                            logger.warning(f"⚠️  Linha {linha_num} ignorada: password é placeholder ({senha})")
                            continue
                        ...
                        logger.debug(f"✅ Linha {linha_num}: {usuario} - credencial válida carregada")
                    else:
                        linhas_ignoradas += 1
                        motivos = []
                        ...
                        logger.warning(f"⚠️  Linha {linha_num} ignorada: {', '.join(motivos)}")
            
            logger.info(f"📊 Carregadas {len(credenciais)} credenciais válidas do arquivo")
            if linhas_ignoradas > 0:
                logger.warning(f"⚠️  Total de {linhas_ignoradas} linhas ignoradas por dados incompletos")
            ...
            raise ValueError(f"Erro de codificação no arquivo. Tente usar encoding diferente.")
        except Exception as e:
            logger.error(f"Erro inesperado ao ler arquivo CSV: {str(e)}")
            raise
    
    def salvar_resultados(self, resultados: List[Dict[str, Any]], 
                         caminho_saida: str,
                         incluir_senha: bool = False) -> None:
        ...
        try:
            ...
            if incluir_senha:
                logger.warning("ATENÇÃO: Senhas foram incluídas no arquivo de saída. "
    ...
    def validar_estrutura_csv(self, caminho_arquivo: str, 
                             colunas_obrigatorias: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            with open(caminho, 'r', encoding=self.encoding, newline='') as arquivo:
                ...
                if not resultado['valido']:
                    resultado['erro'] = f"Colunas obrigatórias não encontradas: {resultado['colunas_faltantes']}"
    ...
    def criar_template_csv(self, caminho_arquivo: str,
                          colunas: List[str] = None,
                          dados_exemplo: Optional[List[Dict[str, str]]] = None) -> None:
    ...
    def salvar_resultados_json(self, resultados: List[Dict[str, Any]], 
        ...
        try:
            ...
            logger.info(f"Resumo: {len(validados_sucesso)} sucessos, {len(validados_erro)} erros")
            logger.warning("ATENÇÃO: O arquivo JSON contém senhas em texto claro. Mantenha-o seguro!")
    ...
    def remover_arquivo_csv(self, caminho_arquivo: str) -> bool:
        ...
        try:
            ...
            if arquivo_path.exists():
                ...
                logger.warning(f"⚠️  Arquivo não encontrado para remoção: {caminho_arquivo}")
                return False
        except Exception as e:
