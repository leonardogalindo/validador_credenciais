...
class LoggingConfig:
    ...
    @classmethod
    def create_file_handler(cls, log_type: str, level: int, 
                           format_type: str = 'detailed') -> logging.FileHandler:
    ...
    @classmethod
    def create_console_handler(cls, level: int, 
                              format_type: str = 'simple') -> logging.StreamHandler:
...
class AppConfig:
    ...
    INCLUDE_PASSWORDS_IN_OUTPUT = os.getenv('INCLUDE_PASSWORDS_IN_OUTPUT', 'false').lower() == 'true'
...
def setup_logging(console_level: Optional[str] = None, 
    ...
    debug_handler = LoggingConfig.create_file_handler('debug', logging.DEBUG, 'detailed')
    root_logger.addHandler(debug_handler)
    
    # 2. Error log - erros e falhas (nível ERROR)
    error_handler = LoggingConfig.create_file_handler('error', logging.ERROR, 'detailed')
    ...
    settings_handler = LoggingConfig.create_file_handler('settings', logging.DEBUG, 'settings')
...
def initialize_app() -> None:
    ...
    settings_logger.debug(f"Diretórios criados: {AppConfig.DATA_DIR}, {LoggingConfig.LOG_DIR}")
