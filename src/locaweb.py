...
class LocawebCredentialValidator:
    ...
    def __init__(self, login_url: Optional[str] = None, timeout: int = 30) -> None:
        ...
        if not self.login_url:
            raise ValueError("URL de login deve ser fornecida via parâmetro ou variável de ambiente LOCAWEB_LOGIN_URL")
        
        logger.debug(f"LocawebCredentialValidator inicializado com login_url: {self.login_url}")
    ...
    def validar_credencial_unica(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Erro inesperado durante validação para usuário {username}: {str(e)}")
            raise
    
    
    def validar_credenciais_em_lote(self, caminho_csv: str, 
                                   incluir_senha_resultado: bool = False) -> List[Dict[str, Any]]:
        ...
        for i, credencial in enumerate(credenciais, 1):
            ...
            try:
                resultado['is_valid'] = self.validar_credencial_unica(username, password)
                if resultado['is_valid']:
                    logger.info(f"✅ SUCESSO {i}/{len(credenciais)} - {username}: Credenciais válidas")
                else:
                    logger.warning(f"❌ FALHA {i}/{len(credenciais)} - {username}: Credenciais inválidas")
            except Exception as e:
    ...
    def salvar_resultados_json(self, resultados: List[Dict[str, Any]], 
        ...
        self.csv_handler.salvar_resultados_json(resultados, caminho_saida, nome_arquivo_origem)


def criar_validador_locaweb(login_url: Optional[str] = None) -> LocawebCredentialValidator:
