...
def descobrir_arquivos_csv() -> List[Path]:
    ...
    logger.info(f"Encontrados {len(arquivos_csv)} arquivo(s) CSV: {[f.name for f in arquivos_csv]}")
    return arquivos_csv
...
def processar_arquivo_csv(arquivo_entrada: Path, validador: Any) -> None:
    ...
    try:
        ...
        arquivo_json = AppConfig.JSON_OUTPUT_DIR / f"relatorio_{arquivo_entrada.stem}.json"
        
        # Salva em JSON (formato principal)
        csv_handler = criar_csv_handler()
        csv_handler.salvar_resultados_json(resultados, str(arquivo_json), arquivo_entrada.name)
        ...
        audit_logger.info(f"BATCH_VALIDATION_END - file: {arquivo_entrada.name}, "
        ...
        audit_logger.error(f"BATCH_VALIDATION_ERROR - file: {arquivo_entrada.name}, error: {str(e)}")
    ...
    if not csv_dir.exists() or not any(csv_dir.glob("*.csv")):
        ...
        try:
            ...
            dados_exemplo = [
                ...
                {
                    'username': 'usuario3@exemplo.com',
                    'password': 'senha789'
                }
            ]
            
            csv_handler.criar_template_csv(
                str(template_path),
                colunas,
                dados_exemplo
            )
...
def main() -> int:
    ...
    try:
        ...
        audit_logger.info(f"APPLICATION_END - processados: {arquivos_processados}, "
