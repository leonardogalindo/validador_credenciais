...
from src.settings import initialize_app, setup_logging, AppConfig, LoggingConfig
...
user3@test.com,senha789"""
        ...
        validador.salvar_resultados_json(resultados, str(json_output), "credenciais.csv")
...
João,joao@test.com"""
    ...
    def test_criacao_template_automatica(self):
        ...
        colunas = [
            'name', 'username', 'email', 'mail_domain', 'cpf', 'rg', 'rg_or_ie', 
            'password', 'password_hash', 'url', 'internal', 'gender', 'address', 
            'phone', 'occupation', 'mother_name', 'company_name', 'cnpj', 
            'stealer_family', 'infection_date', 'hardware_id', 'hostname', 
            'malware_installation_path', 'ip', 'user_agent', 'brand', 'source', 
            'threat_id', 'breach_date'
        ]
        
        dados_exemplo = [{col: f"exemplo_{col}" if col not in ['username', 'password'] 
...
user2@test.com,senha456"""
    ...
    def test_integracao_settings_com_aplicacao(self):
        ...
        with patch.dict('os.environ', {
            ...
            assert settings['api']['locaweb_login_url'] == 'https://integration.test.com'
