...
class TestLocawebCredentialValidator:
    ...
    def test_init_com_timeout_customizado(self):
        ...
        validator = LocawebCredentialValidator(login_url="https://test.com", timeout=60)
    ...
    def test_validar_credencial_unica_falha(self, mock_post):
        ...
        resultado = self.validator.validar_credencial_unica(
            "user@test.com", "senha_errada"
        )
    ...
    def test_validar_credencial_unica_parametros_vazios(self):
        ...
        with pytest.raises(
            ValueError, match="Username e password não podem estar vazios"
        ):
...
user1@test.com,senha123"""
        ...
        resultados = self.validator.validar_credenciais_em_lote(
            str(csv_file), incluir_senha_resultado=False
        )
...
user2@test.com,senha456"""
    ...
    def test_salvar_resultados_json(self):
        ...
        resultados = [{"username": "user1@test.com", "is_valid": True, "error": None}]

        json_file = self.temp_dir / "resultados.json"

        # Mock do csv_handler
        with patch.object(
            self.validator.csv_handler, "salvar_resultados_json"
        ) as mock_salvar:
            self.validator.salvar_resultados_json(
                resultados, str(json_file), "origem.csv"
            )
...
user2@test.com,senha456"""
        ...
        def mock_post_side_effect(url, data, **kwargs):
            ...
            mock_response.status_code = (
                200 if data["username"] == "user1@test.com" else 401
            )
            return mock_response
