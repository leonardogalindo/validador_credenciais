...
from src.menu import MenuValidador, TipoEmpresa, criar_menu, executar_selecao_empresa
...
class TestTipoEmpresa:
    ...
    def test_valores_enum(self):
        ...
        assert TipoEmpresa.BLING.value == "bling"
...
class TestMenuValidador:
    ...
    def test_inicializacao(self):
        ...
        assert len(self.menu.opcoes) == 2
        assert '1' in self.menu.opcoes
        assert '2' in self.menu.opcoes
        ...
        opcao_bling = self.menu.opcoes['2']
        assert opcao_bling['nome'] == 'Bling'
        assert opcao_bling['tipo'] == TipoEmpresa.BLING
        assert 'descricao' in opcao_bling
        assert 'factory' in opcao_bling
    ...
    def test_obter_escolha_usuario_opcao_invalida(self, mock_print, mock_input):
        ...
        mock_print.assert_any_call("❌ Opção inválida! Por favor, escolha uma opção válida.")
    ...
    @pytest.mark.unit
    @patch('src.menu.criar_validador_bling')
    def test_criar_validador_bling(self, mock_criar_validador):
        """Testa criação de validador Bling."""
        mock_validador = Mock()
        mock_criar_validador.return_value = mock_validador
        
        validador = self.menu.criar_validador('2')
    ...
    def test_listar_opcoes_disponiveis(self):
        ...
        assert opcoes == {'1': 'Locaweb', '2': 'Bling'}
    ...
    @patch('builtins.print')
    def test_executar_menu_interativo_sucesso(self, mock_print, mock_criar_validador, 
                                            mock_obter_escolha, mock_exibir_menu):
    ...
    def test_executar_menu_interativo_cancelado(self, mock_obter_escolha, mock_exibir_menu):
    ...
    @pytest.mark.unit
    @patch.object(MenuValidador, 'exibir_menu')
    @patch.object(MenuValidador, 'obter_escolha_usuario', side_effect=Exception("Erro no menu"))
    @patch('builtins.print')
    def test_executar_menu_interativo_erro(self, mock_print, mock_obter_escolha, mock_exibir_menu):
...
class TestMenuIntegration:
    ...
    @patch('builtins.print')
    @patch('src.menu.criar_validador_locaweb')
    def test_fluxo_completo_selecao_locaweb(self, mock_criar_validador, mock_print, mock_input):
    ...
    @patch('builtins.print')
    @patch('src.menu.criar_validador_bling')
    def test_fluxo_completo_selecao_bling(self, mock_criar_validador, mock_print, mock_input):
        """Testa fluxo completo de seleção do Bling."""
        mock_validador = Mock()
        mock_criar_validador.return_value = mock_validador
